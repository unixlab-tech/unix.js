...
export class Logger {
   ...
   */
  public info(saveAsFile: boolean, ...args: unknown[]) {
    const now = new Date();
    ...
    if (saveAsFile) this.file("info", ...args)
  }
  /**
   ...
   * This example initializes a new Logger object with the PRETTY design option and specifies the log directory as "./logs". It then calls the info method with the message "This is an information message". The formatted log message will be displayed in the console and saved to a file in the specified directory.
   */
  public warn(saveAsFile: boolean, ...args: unknown[]) {
    const now = new Date();
    ...
    if (saveAsFile) this.file("warn", ...args);
  }
  /**
   ...
   * This example initializes a new Logger object with the PRETTY design option and specifies the log directory as "./logs". It then calls the info method with the message "This is an information message". The formatted log message will be displayed in the console and saved to a file in the specified directory.
   */
  public error(saveAsFile: boolean, ...args: unknown[]) {
    const now = new Date();
    ...
    if (saveAsFile) this.file("error", ...args);
  }
  ...
  public file(level: "info" | "warn" | "error", ...args: unknown[]): void {
    const now = new Date();
    const filePath = this.config && this.config.directory
      ? path.resolve(
